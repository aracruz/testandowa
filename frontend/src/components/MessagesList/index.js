    ...
    if (message.mediaType === "eventMessage") {
      ...
      try {
        ...
        if (!dataJsonString || typeof dataJsonString !== "string") {
          console.error(
            "dataJson não está definido ou não é uma string válida."
          );
          return;
        }
        const parsedData = JSON.parse(dataJsonString);
        const viewOnceMessage =
          parsedData.message.viewOnceMessage.message.interactiveMessage; // Acessando a mensagem diretamente
        if (!viewOnceMessage) {
        ...
        const botoes =
          viewOnceMessage.nativeFlowMessage?.buttons?.map((button) => {
        ...
        console.error(
          "Erro ao processar a mensagem do tipo viewOnceMessage: ",
          error
        );
      }
    } else if (
